...
def build_optimizer_our(model, optimizer_cfg):
    from torch.optim import Adam
    min_lr = optimizer_cfg.lr / 100
    optim = Adam([
        {'params': (param for name, param in model.named_parameters() if param.requires_grad), 'lr': min_lr},
    ])
    return optim
